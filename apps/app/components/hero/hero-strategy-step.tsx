    ...
    >
      ...
      <div className="flex-1 space-y-3">
        {[
          ...
          { title: "Highlight unlimited seat structure", color: "text-emerald-600", bg: "bg-emerald-50" }
        ].map((opp, i) => (
          <motion.div
            key={i}
          ...
          >
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-lg ${opp.bg} flex items-center justify-center shrink-0`}>
                <Sparkles className={`w-5 h-5 ${opp.color}`} />
              </div>
              <div>
                <div className="font-medium text-slate-900">{opp.title}</div>
                <div className="text-xs text-slate-500">High confidence based on competitor data</div>
              </div>
            </div>
