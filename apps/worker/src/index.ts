...
import { QUEUE_NAMES, RunExecuteJob, StageExecuteJob, RunCancelJob, StageReplayJob, DeadLetterReviewJob } from '@flank/shared';
...
function start() {
  ...
  const runWorker = new Worker(QUEUE_NAMES.RUN_EXECUTE, async (job) => {
    const { runId, targetId, requestedBy } = job.data as RunExecuteJob;
    await initializeRun(runId, targetId, requestedBy || 'system');
  }, { connection, concurrency });

  const stageWorker = new Worker(QUEUE_NAMES.STAGE_EXECUTE, async (job) => {
  ...
  const stageReplayWorker = new Worker(QUEUE_NAMES.STAGE_REPLAY, async (job) => {
  ...
  const runCancelWorker = new Worker(QUEUE_NAMES.RUN_CANCEL, async (job) => {
    const { runId, requestedBy } = job.data as RunCancelJob;
    await handleRunCancellation(runId, 'Cancelled via API');
  }, { connection, concurrency });

  const deadLetterWorker = new Worker(QUEUE_NAMES.DEAD_LETTER_REVIEW, async (job) => {
