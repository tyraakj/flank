...
export const providerMetrics = {
  recordSearch(provider: string, query: string, latencyMs: number, cached: boolean) {
    console.log(
      `[Metrics:Search] ${provider} | Query: "${query}" | Latency: ${latencyMs}ms | Cached: ${cached}`,
    );
  },
  ...
  recordPageRead(
    ...
    console.log(
      `[Metrics:PageRead] ${provider} | URL: ${url} | Latency: ${latencyMs}ms | Cached: ${cached} | Size: ${sizeBytes} bytes`,
    );
  },
  ...
  recordLlmGeneration(
    ...
    console.log(
      `[Metrics:LLM] ${provider}:${model} | Latency: ${latencyMs}ms | Tokens (P/C/T): ${promptTokens}/${completionTokens}/${promptTokens + completionTokens}`,
    );
  },
