...
14. **Model tiering per graph node.** High-throughput extraction and classification nodes (`Profiler`, `Discovery`, `Verifier`, `Pricing`, `Feature`, `Positioning`) run on fast, cost-effective models (`gemini-2.0-flash`). High-reasoning models (`gemini-1.5-pro` / `gemini-2.0-pro`) are allocated strictly to synthesis (`Strategist`) and quality gates (`Critic`).
15. **Progressive UI pipeline streaming.** While downstream synthesis nodes wait at barrier joins, the frontend does not block. Completed parallel extraction stages immediately publish `STAGE_SUMMARY` SSE events to progressively hydrate pricing, feature, and positioning matrices in real-time.
