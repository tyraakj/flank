...
export const PricingPlanSchema = z.object({
  name: z.string().describe("Verbatim official plan label (e.g., 'Starter', 'Pro', 'Enterprise', 'Free')."),
  band: PlanBandSchema.describe("Standardized plan tier category: FREE, GROWTH, ENTERPRISE, or CUSTOM."),
  amount: z
    .number()
    .nullable()
    .describe("Exact numeric price value per unit/seat. Null if contact sales or unpublished. Never infer amounts from discounts or ranges."),
  currency: z
    .string()
    .default("USD")
    .describe("Standard ISO currency code (e.g., 'USD', 'INR', 'EUR', 'GBP'). Required if amount is numeric."),
  interval: BillingIntervalSchema.describe("Billing cadence: MONTHLY, YEARLY, or ONE_TIME. Required if amount is numeric."),
  seatModel: z
    .string()
    .default("unknown")
    .describe("Monetization unit (e.g., 'per user/month', 'per seat/mo', 'flat rate', 'consumption/credits', 'unlimited', 'unknown')."),
  usageLimits: z
    .array(z.string())
    .default([])
    .describe("Extracted usage quotas or limits (e.g. ['Up to 5 users', '10,000 API calls/month', '100 GB storage'])."),
  includedFeatures: z
    .array(z.string())
    .default([])
  ...
  sourceUrl: z.string().url().describe("Canonical URL of the pricing page or source where this tier was found."),
  rawPriceString: z
    .string()
    .nullable()
    .optional()
    .describe("Verbatim price string as displayed on the page (e.g., '$49/mo billed annually', 'Custom quote')."),
  excerpt: z
    .string()
    .describe("Exact verbatim excerpt from the pricing table, card, or prose for auditable evidence."),
  confidence: z
    .number()
    .min(0)
    .max(100)
...
export const CompetitorPricingExtractionSchema = z.object({
  ...
  availabilitySummary: z
    .enum(["published", "contact-sales", "free-only", "trial-only", "not-published", "inaccessible"])
    .describe("Overall pricing availability posture for this competitor."),
  pricingModelType: z
    .string()
    .describe("High-level pricing architecture (e.g., 'Tiered Per-Seat Subscription', 'Freemium', 'Usage-Based', 'Enterprise Quote Only')."),
  trialAvailable: z
    .boolean()
    .default(false)
    .describe("Whether a free trial is offered."),
  trialDays: z
    .number()
    .nullable()
    .optional()
